...
export const _useGlobalSettings = () => {
  ...
  const gettingGlobalSettings = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${userApi}/g/settings`, { withCredentials: "include" });
      if (res.status === 201) {
  ...
  const onFinish = async () => {
    setLoading(true);
    try {
      const res = await axios.put(`${userApi}/g/settings`, _settings, { withCredentials: "include" });
      if (res.status === 200) {
...
export const _useQuizSettings = (quizId) => {
  ...
  const fetchingQuizSettings = async (x) => {
    setLoading(true);
    try {
      const res = await axios.get(`${quizApi}/s/${x}`, { withCredentials: "include" });
      if (res.status === 200) {
  ...
  const addQuizSettings = async (x) => {
    ...
    try {
      const res = await axios.put(`${quizApi}/s/${x}`, _settings, { withCredentials: "include" });
      if (res.status === 200) {
        toast.success(res.data.message);
      }
    } catch (error) {
